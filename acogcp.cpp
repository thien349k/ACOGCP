...
{
    //Khởi tạo thuộc tính và kiểm tra sự tồn tại của tập tin.
    filename=str;
    ifstream inData(str);
    if(inData.fail())
    {
    ...
    //Tìm bậc tối đa, tổng của bậc hiện tại so sánh với bậc tối đa
    int sum,max=0;
...
void ACOGCP::compute()
{
    ...
    for (j = 1; j <vertex; j++)
    {
        ...
        for (k = 0; k < vertex; k++)
        {
            if (j!=k)
            {
                value=adjmat[j][k]*color[k];// tính toán xung đột
                if (value==color[j])
                {
                    if (color[k]<=max_deg1)
                        color[k]=color[k]+1;
                    else
                        color[k]=color[k]%(+max_deg1)+1;//phép toán modulo. Chia lấy dư + 1
                    j=0;
                    k=0;
                }
            }
...
int ACOGCP::countDistinct()//#
{
	...
	for (int i = 0; i < vertex; i++)
    { 
		if (s.find(color[i]) == s.end())//nếu màu chưa được sử dụng thì tăng thêm 1 màu
        { 
			s.insert(color[i]); 
			res++;
		} 
...
//
{
    for(int i=0;i<(vertex-1);i++)//vì có 1 đỉnh là tham chiểm không cần ss
    {
...
//hàm này để lưu trữ thông tin về thuật toán
{
    fstream fstream_ob;
